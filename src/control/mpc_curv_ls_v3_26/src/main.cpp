                     ...
                     tf_listener_(std::make_shared<tf2_ros::TransformListener>(*tf_buffer_))
{
    ...
    this->declare_parameter<double>("steering_delay", 0.1);
    ...
    weights_pub_ = this->create_publisher<std_msgs::msg::Float64MultiArray>("mpc/weights", 10);
    info_pub_ = this->create_publisher<std_msgs::msg::String>("mpc/info", 10);
    ...
    weights_msg.data = cost_weights;
    weights_pub_->publish(weights_msg);
    ...
    char buffer[256];
    snprintf(buffer, sizeof(buffer), "MPC Node \"mpc_curv_ls_v3\" initialized with: trajectory: %s, v_x_ref: %.2f", traj_file.c_str(), v_x_ref);
    info_msg.data = std::string(buffer);
...
void MPCNode::solveMPC()
{
    ...
    // Delay compensation for cmds
    double total_delay = steering_delay_ + solver_time.mean;

    State state_delay = sim_car(current_state_, current_controls_, total_delay); // TODO: Not the best way to do it,
    ...
    if (status != 0)
    {
        ...
        steering_cmd = 0.0;
        speed_cmd = 0.0;
        current_controls_.delta_dot = 0.0;
        current_controls_.vx_dot = 0.0;
    }
    else
    {
        ...
        current_controls_.delta_dot = control_output_1[0];
        current_controls_.vx_dot = control_output_1[1];
    ...
    double total_time=(this->now()-init_time).seconds(); 
    solver_time.update(total_time);
...
)
{
    ...
    double dt = 0.01; // Passo fino para melhor precisão
    int steps = std::ceil(delay / dt);

    // Variáveis locais para simulação
    double s = state.s;
    double n = state.n;
    double mu = state.u; // mu = theta (erro de orientação)
    double v = state.vx;
    double delta = state.delta;



    // Inputs (Mantêm-se constantes durante o delay - Zero Order Hold)
    double accel_cmd = control.vx_dot;
    double steer_rate_cmd = control.delta_dot;

    for (int i = 0; i < steps; i++)
    {
        // 1. Calcular variaveis auxiliares
        double tan_delta = std::tan(delta);
        double cos_mu = std::cos(mu);
        double sin_mu = std::sin(mu);

        // Yaw Rate Cinemático (r = v/L * tan(delta))
        double yaw_rate = (v / L) * tan_delta;

        // Obter curvatura de referência kappa no ponto s
        int idx = s/reference_trajectory_.s[1]; // Aproximação do índice
        double kappa = reference_trajectory_.kappa[idx];

        // 2. Dinâmica de Frenet (Consistente com CasADi)
        // Evitar divisão por zero no denominador (1 - k*n)
        double den = 1.0 - kappa * n;
        if (std::abs(den) < 0.05)
            den = 0.05; // Proteção simples

        double s_dot = (v * cos_mu) / den;
        double n_dot = v * sin_mu;
        double mu_dot = yaw_rate - (kappa * s_dot); // Variação do erro de angulo

        double v_dot = accel_cmd;
        double delta_dot = steer_rate_cmd;

        // 3. Integração de Euler (x = x + dx * dt)
        s += s_dot * dt;
        n += n_dot * dt;
        mu += mu_dot * dt;
        v += v_dot * dt;
        ...
        // Opcional: Atualizar kappa baseado no novo s se tiveres acesso ao mapa aqui
    ...
    // O yaw rate final deve ser recalculado com o novo v e delta
    new_state.r = (v / L) * std::tan(delta);
    return new_state;
}
