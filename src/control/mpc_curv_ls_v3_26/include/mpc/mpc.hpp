...
private:
    ...
    State sim_car(const State &state, const Control &control, double delay);
    ...
    rclcpp::Publisher<std_msgs::msg::Float64MultiArray>::SharedPtr weights_pub_;
    ...
    struct SolveTimer {
        double mean = 0.01; // Começa com uma estimativa razoável (10ms)
        double alpha = 0.1; // Fator de suavização (0.1 = média das ultimas ~10 iterações)

        void update(double new_time) {
            if (mean == 0.0) {
                mean = new_time; // Primeira vez
            } else {
                // Fórmula EMA: Média = (Valor_Novo * alpha) + (Média_Antiga * (1 - alpha))
                mean = (new_time * alpha) + (mean * (1.0 - alpha));
            }
        }
    };

    // Declaração global ou na classe
    SolveTimer solver_time;
};
