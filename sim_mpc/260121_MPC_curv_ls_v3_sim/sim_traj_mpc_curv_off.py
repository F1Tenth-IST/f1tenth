#!/usr/bin/env python3
"""
Closed-loop MPC Simulation for F1TENTH Vehicle (acados-based)
This script provides a Python reimplementation of a MATLAB closed-loop MPC simulation for an F1TENTH vehicle, using acados for real-time optimal control. It is designed to be modular and configurable, supporting YAML-based parameter files for both the vehicle and the MPC controller.
Key Features:
-------------
- Loads a track from a CSV file containing [s, x, y, kappa, (nl), (nr)] and allows for optional override of track width.
- Loads controller and vehicle parameters from YAML files (see --mpc_config and --car_config).
- Sets up the acados OCP (Optimal Control Problem) using your custom model and constraints.
- Runs a closed-loop simulation, applying the MPC at each timestep, with support for actuation/input delay.
- Provides real-time and post-simulation plotting of vehicle states, control inputs, and track boundaries.
- Logs simulation data and supports lap timing and multi-lap runs.
- Includes CLI flags for configuration, plotting, and simulation time.
How to Run:
-----------
Example command:
    python sim_traj_mpc_curv_off.py \
        --csv path/to/track.csv \
        --mpc_config path/to/mpc_config.yaml \
        --car_config path/to/car_model.yaml \
Arguments:
    --csv           Path to the track CSV file (default: a sample track in ./traj).
    --mpc_config    Path to the YAML file with MPC parameters.
    --car_config    Path to the YAML file with vehicle parameters.
    --sim-time      Total simulation time in seconds (default: 40).
    --track-width   Override the track half-widths (nl, nr) with a constant value.
    --no-plot       Disable real-time plotting/animation.
Typical Workflow:
-----------------
1. Prepare your track CSV and parameter YAML files.
2. Run the script with the desired arguments.
3. The script will load the track and parameters, initialize the MPC, and start the simulation loop.
4. During simulation, you can pause or stop with keyboard input.
5. After simulation, state and input trajectories are plotted for analysis.
Dependencies:
-------------
- numpy, matplotlib, casadi
- acados_settings.py, bicycle_model.py, and other project-specific modules
Note:
-----
- The script is intended for research and development with F1TENTH or similar vehicle models.
- Ensure your YAML and CSV files are correctly formatted and compatible with your model.
- For best results, use in a terminal that supports interactive input and plotting.
"""
...
class MPCSim:
    ...
    def __init__(
        ...
        self.constraint, self.ocp, self.model, self.solver, self.params_ns = acados_settings(
            self.s_ref,
            self.kappa_ref,
            self.d_left,
            self.d_right,
        ...
        # 3.1) Criar simulador acados para integração
        self.sim_solver = self._create_acados_simulator()
    ...
    def _create_acados_simulator(self) -> AcadosSimSolver:
        """Cria um simulador acados para integração do estado."""
        sim = AcadosSim()

        # Criar um AcadosModel com as mesmas variáveis/f_expl do modelo CasADi usado no OCP
        acados_model = AcadosModel()
        acados_model.x = self.model.x
        acados_model.u = self.model.u
        acados_model.z = self.model.z
        acados_model.p = self.model.p
        acados_model.f_expl_expr = self.model.f_expl_expr
        acados_model.name = self.model.name
        print("Creating AcadosSim with model:", self.model.name)
        ...
        # Parâmetros
        sim.parameter_values = self.p_vec
    ...
    # -------------- integração com acados --------------
    def _integrate_one_step(self, x: np.ndarray, u: np.ndarray) -> np.ndarray:
        """
        Integra usando o simulador acados (RK4).
        """
        self.sim_solver.set("x", x)
        self.sim_solver.set("u", u)
        self.sim_solver.set("p", self.p_vec)
        
        status = self.sim_solver.solve()
        if status != 0:
            print(f"[WARN] Simulador acados falhou com status {status}")
        
        x_next = self.sim_solver.get("x")
        return x_next.astype(float)
    ...
    def run(self, sim_time: float) -> SimResult:
        ...
        for k in range(n_steps):
            ...
            if status != 0:
                ...
                u_applied = uN[0]
                # Usar sempre o simulador acados para propagar o estado
                x_next = self._integrate_one_step(self.x, u_applied)
...
def main():
    ...
    ap.add_argument("--save-workspace", action="store_true", help="Save simulation workspace to file")
    args = ap.parse_args()
    ...
    FORCE_SAVE = False # True # usually False, True is for debugging
    if args.save_workspace or FORCE_SAVE:
        # save_workspace( "workspace_sim_traj_mpc.pkl" )  # does not work, modules not picklable
        # save_workspace( "workspace_sim_traj_mpc.pkl", variables={ "args": args, "overrides": overrides, "x_init": x_init, "res": res } )
        # save_workspace() # autofilename, all variables (except unpicklable)
        save_workspace(filename="", variables={"args": args, "overrides": overrides, "x_init": x_init, "res": {"x_hist": res.x_hist, "u_hist": res.u_hist, "status": res.status, "solve_ms": res.solve_ms, "dt": res.dt, "freq": res.freq}})


    # pós-sim (figuras bloqueantes, só no fim)
    if args.no_plot:
        pass   # or just skip plotting
    else:
